...
次の図は、並行（Concurrency）と並列（Parallelism）という二つの概念の違いを、シェフの調理に例えて対比しています。

![並行と並列](../../assets/diagrams/chapter03/3-7-concurrency-parallelism.svg)

ここで描かれているのは、一人のシェフが複数の料理を「切り替えながら」同時進行させる並行性と、複数のシェフが実際に「同時に」異なる料理を進める並列性の本質的な違いです。並行はタスクをどのように構造化するかという設計の問題であり、シングルコアのマシンでも効果を発揮します。この区別を理解することが、async/awaitやスレッドを適切に使い分けるための出発点となります。

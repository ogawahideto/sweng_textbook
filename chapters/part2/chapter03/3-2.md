...
コードを書き始める前に、制御フローを頭の中で（あるいは紙の上で）設計します。設計には三つのステップがあります。
...
**異常系を列挙する**
幹が定まれば、次は「枝」——すなわち異常系に目を向けます。「何が失敗しうるか」を洗い出していきましょう。異常系は正常系の「枝」として扱います。幹と枝を分離することで、術式の見通しが良くなります。

**制御フローを図式化する**
構造が整ったら、図に落とし込んで全体を俯瞰する習慣をつけましょう。複雑な処理は、書く前に図にすると整理できます。
...
**分岐の技法**
条件分岐は術式の基本ですが、最も複雑さが生まれやすい場所でもあります。

- **ガード節（早期リターン）**: 異常系を関数の冒頭で弾き、正常系をメインの流れとして残す技法です。正常系がネストせず、左端に揃うため、見通しが劇的に良くなります。
- **肯定形を優先する**: 人間の脳は否定より肯定を処理しやすいです。`if not user.is_inactive():` よりも `if user.is_active():` と書きましょう。
- **複雑な条件は名前をつける**: 条件式が長くなったら、`is_eligible` のように意図を表す名前を変数やメソッドに抽出します。
- **switch/match の活用**: 分岐先が3つ以上あり、同じ変数を比較する場合は Python の `match` 文などが適しています。
...
分岐の技法に続き、コードを繰り返し実行させる「ループ」の使い分けを見ていきます。
...
分岐とループで「通常の流れ」を制御できるようになれば、次に備えるべきは想定外の事態——例外——への対処です。プログラムの実行中に異常が発生したとき、どのように対処するかを決める仕組みが例外処理です。例外処理を理解するには、**制御の流れ**（どこで・どう対処するか）と**エラー情報の渡し方**（何を使って伝えるか）の2つの軸で整理すると見通しが良くなります。

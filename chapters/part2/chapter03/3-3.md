...
「一つの仕事」が定まったら、次はその仕事の「入口」と「出口」をどう設計するかが問われます。
...
契約が明確になったら、関数の「性質」という観点から、もう一段深く捉えてみましょう。
...
純粋関数と副作用の分離が意識できたら、もう一つ関数を使いこなすうえで欠かせない概念——スコープを押さえておきましょう。
...
関数で処理を整理できるようになったら、今度はデータと処理をひとまとめにするより大きな器——クラス——に目を向けましょう。
...
フィールドとメソッドの役割を把握したら、次はオブジェクトが「どのように生まれるか」を定める初期化の仕組みを見ていきましょう。
...
Pythonでのクラスの書き方が掴めたところで、この「データと振る舞いをまとめる」という概念が他の言語でどのように表現されるかも知っておくと、多言語の設計を読む力が広がります。
...
公開範囲の制御に加え、モジュールには名前の衝突を防ぐというもう一つの重要な役割があります。

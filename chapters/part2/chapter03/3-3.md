...
次の図は、関数・クラス・モジュールという三層の「器」が処理をどのように包み込んでいるかを示しています。

![術式の器](../../assets/diagrams/chapter03/3-3-encapsulation-pot.svg)

ここで表現されているのは、小さな器（関数）が中くらいの器（クラス）に収まり、さらに大きな器（モジュール）が全体を包む入れ子構造です。各層では異なる単位の「関心の分離」が行われており、最も内側の関数が「一つの仕事」に集中することで、外側の器も自然と整理されます。この階層的な包含関係がコードの可読性と再利用性を支える骨格となります。

---

## 関数（Function）: 最小の器

最も小さな器である関数から出発し、クラス、モジュールという順に器を大きくしていきましょう。
...
「一つの仕事」が定まったら、次はその仕事の「入口」と「出口」をどう設計するかが問われます。関数の入口（引数）と出口（戻り値）を明確にすることは、関数の「契約」を読み手に示すことです。
...
契約が明確になったら、関数の「性質」という観点からもう一段深く捉えてみましょう。関数には2つの種類があります。
...
純粋関数と副作用の分離が意識できたら、もう一つ関数を使いこなすうえで欠かせない概念——スコープを押さえておきましょう。変数や関数には「見える範囲（スコープ）」があります。
...
関数で処理を整理できるようになったら、今度はデータと処理をひとまとめにするより大きな器——クラス——に目を向けましょう。関数が「処理」の器なら、クラスは「データとそれに関連する処理」をひとまとめにする器です。
...
フィールドとメソッドの役割を把握したら、次はオブジェクトが「どのように生まれるか」を定める初期化の仕組みを見ていきましょう。`__init__` メソッド（コンストラクタ）は、オブジェクトが生まれる瞬間に呼ばれる特別なメソッドです。ここで、オブジェクトが正しい状態で生まれることを保証します。
...
Pythonでのクラスの書き方が掴めたところで、この「データと振る舞いをまとめる」という概念が他の言語でどのように表現されるかも知っておくと、多言語の設計を読む力が広がります。言語によって異なる姿をとります。
...
関数とクラスを整理できたら、さらに大きな視点——複数のクラスや関数をどのファイルにまとめ、外部にどこまで公開するかという「モジュール」の設計に目を向けましょう。
...
公開範囲の制御に加え、モジュールには名前の衝突を防ぐというもう一つの重要な役割があります。モジュールは「名前空間」としても機能します。同じ名前の要素が衝突することを防ぎます。
...
名前空間の仕組みが分かったところで、QuestForgeを例にモジュールが実際のプロジェクトでどのように組み合わさるかを見てみましょう。

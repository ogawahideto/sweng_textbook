...
テキストをそのままDBに入れても、意味による検索はできません。まず**エンベディング（Embedding）**と呼ばれる変換で、テキストを高次元の数値ベクトルに変換します。なお、Anthropic自身はエンベディング専用APIを提供しておらず、公式には[Voyage AI](https://www.voyageai.com/)などの専用モデルの利用を推奨しています。ここでもVoyage AIのSDKを使います。
...
def embed_text(text: str) -> list[float]:
    ...
    result = vo.embed([text], model="voyage-3.5")

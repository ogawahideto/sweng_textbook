...
ここまで、テストの全体像（5.1節）とテスト設計の技法（5.2節）を学びました。しかし、これらはいずれも「コードを書いた後にテストを書く」という前提に立っています。
...
![TDDサイクル](../../assets/diagrams/chapter05/5-3-tdd-cycle.svg)
...
テストという安全網があるので、安心してコードを磨けます。第4章で学んだリファクタリングの技法が、ここで活きます。
...
第4章との関係が明確になります。TDDの3番目のフェーズ「Refactor」は、**テストという安全網があるからこそ安心して行える**のです。第4章で「テストが不可欠」と示唆されていた理由が、ここで具体的に体験できます。
...
3. **リファクタリングの安全網**: TDDで書いたテストが、第4章で学んだリファクタリングを安心して行うための基盤になる。
...
- 接続: 4章（リファクタリングの安全網）→5.3（TDD）→5.4（AIがテストを生成）

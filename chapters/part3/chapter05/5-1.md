...
第4章で、あなたはコードを美しく磨く「彫刻」の技法を学びました。しかし、どれほど美しい彫刻も、明日の変更で壊れないという保証はありません。磨いた彫刻を守り続けるには——それを安心して変え続けるには——**テスト**という守護魔法が必要です。

テストの世界に初めて足を踏み入れると、膨大な種類の手法やツールに圧倒されるかもしれません。単体テスト、結合テスト、E2Eテスト、回帰テスト……。しかし、これらは無秩序に並んでいるわけではありません。明確な「階層構造」を持っています。
...
システム全体を通して、ユーザーの視点で「仕様通りに動くか」を検証します。受入テストは、第1章で定義した要求やユースケースが満たされているかを確認するテストでもあります。要求の定義（1章）からテストの検証（5章）まで、本書全体が一つの開発サイクルとして繋がっています。
...
![テストピラミッド](../../assets/diagrams/chapter05/5-1-test-pyramid.svg)
...
ピラミッドの形が逆さま（上層ばかり多い）になると、テストの実行が遅く、失敗原因の特定も困難になります。**土台（単体テスト）を厚く、頂点（E2E）は薄く**が原則です。
...
![V字モデル](../../assets/diagrams/chapter05/5-1-v-model.svg)
...
- 接続: 1章（要求定義）→5章（受入テスト）、2章（DIP）→テストダブル、4章（リファクタリング）→テストの安全網

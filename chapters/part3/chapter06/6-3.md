...
まず、認証の要である「パスワードの安全な保管」から見ていきましょう。
...
パスワードを守る方法を押さえたら、次は「誰がログインしているか」をサービス間で安全に伝えるトークンの仕組みを学びましょう。
...
この脅威に対応するため、まず自動チェックツールを活用する方法を押さえましょう。
...
では、なぜSBOMが現代のセキュリティに欠かせないのか、具体的な事例から確認しましょう。
...
重要性が明らかになったところで、SBOMをCI/CDに組み込む具体的なフォーマットと手順に踏み込みましょう。
...
XSSの対策が整ったら、もう一つの古典的な攻撃パターンであるCSRFへの備えも加えましょう。
